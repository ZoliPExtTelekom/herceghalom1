...
def _room1(rt: dict[str, Any]) -> None:
    ...
    spikes = {"id": "spikes_1", "type": "spikes", "x": 540, "y": 120, "w": 220, "h": 80}
    rt["puzzle"] = {"plate_a": plate_a, "plate_b": plate_b, "spikes": spikes, "hold_t": 0.0}
    rt["entities"] = [plate_a, plate_b, spikes, {"type": "door", "x": 885, "y": 240, "w": 30, "h": 80}]
...
def _room1_tick(room: Any, dt: float) -> None:
    ...
    spikes = rt["puzzle"]["spikes"]

    phase = (room.tick % 40) / 40.0  # 2s cycle at 20Hz
    spikes_active = phase < 0.5
    spikes["active"] = spikes_active
    ...
    if spikes_active:
        for ps in room.players.values():
            if _player_in_rect(ps, spikes):
                _damage(room, ps, 1, "spikes_1", cooldown_s=0.35)
